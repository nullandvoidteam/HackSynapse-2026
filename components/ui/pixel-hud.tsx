...
interface XPBarProps {
  /** Current XP value */
  current: number
  /** Maximum XP for the current level */
  max: number
  /** Number of pixel segments to divide the bar into */
  segments?: number
  label?: string
  className?: string
  animate?: boolean
 ...
 * Matches the "LEVEL 12" progress bar style from the login page reference.
 */
...
/**
 * LevelBadge — Full LEVEL N + XP display widget.
 * Matches the dark HUD panel in the login page reference.
 */
export function LevelBadge({ level, xp, maxXp = 500, className }: LevelBadgeProps) {
  return (
    <motion.div
      ...
      className={cn(
        "bg-[#191535]/95 border-2 border-[#382f6b] rounded-xl p-3.5 shadow-2xl backdrop-blur-md min-w-[160px]",
        className
      )}
    >
...
/**
 * PixelStreakCounter — Animated 🔥 streak day counter.
 */
export function PixelStreakCounter({ days, className }: StreakProps) {
  return (
    <motion.div
      ...
      className={cn(
        "flex items-center gap-2 bg-orange-500/10 border-2 border-orange-400/60 rounded-xl px-3 py-2 cursor-default select-none",
        className
      )}
    >
      <motion.span
...
/**
 * AchievementBadge — Gamified pill/badge.
 * Matches "QUEST COMPLETE ✓" and "BUG HUNTER 🐛" from the login reference.
 */
export function AchievementBadge({ icon, label, variant = "success", className, animate = true }: BadgeProps) {
  return (
    <motion.div
      ...
      className={cn(
        "inline-flex items-center gap-1.5 border-2 text-white px-3 py-1.5 rounded-lg font-pixel text-[9px] font-bold tracking-wider uppercase select-none",
...
/**
 * XPPill — The hovering "+120 XP" pill style pill shown in the scene.
 */
