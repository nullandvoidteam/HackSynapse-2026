...
export interface ButtonProps extends Omit<HTMLMotionProps<"button">, "size"> {
  ...
  size?: "sm" | "md" | "lg"
  /** Show a shimmer effect on hover */
  shimmer?: boolean
  /** formAction for use inside Server Component forms */
...
const GamifiedButton = React.forwardRef<HTMLButtonElement, ButtonProps>(
  ({ className, variant = "primary", size = "md", shimmer = false, children, ...props }, ref) => {
    const baseStyles =
      ...
      "disabled:opacity-50 disabled:pointer-events-none rounded-xl overflow-hidden select-none cursor-pointer"
    ...
    const variantStyles: Record<string, string> = {
      ...
      outline:   "bg-white text-gray-700 border-[#e5e5e5] border-2 hover:bg-gray-50",
    }
